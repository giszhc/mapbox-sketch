...
`example/` 中的绘制 demo 在前端自动切片。配置合并服务地址后会上传 ZIP 并由独立服务合成大图；地址留空或服务不可用时会下载包含切片和清单的 ZIP。后端地址在绘制页面中配置，不需要前端项目内包含 Node 服务。
...
填写服务根地址即可，**不要**在末尾加 `/merge` 或 `/health`。Demo 会先请求 `GET /health` 检查服务；大图被前端自动切成多块后，再将 ZIP 上传到 `POST /merge`。只有需要拼合切片时才会调用服务。未填写地址、探活失败或合并失败时，Demo 会下载带有切片和清单的 ZIP。
